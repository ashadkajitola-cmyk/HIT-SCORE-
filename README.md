...
<script>
        ...
        db.ref('users/' + myUserId).once('value', (snapshot) => {
            if(!snapshot.exists()) {
                db.ref('users/' + myUserId).set({
                    points: 50, // Naye user ko 50 points free
                    subscription: null,
                    subExpiry: 0
                });
            }
    ...
    // Password hata diya gaya hai: Sirf Active Subscription hone par admin panel open hoga
    ...
    // User ID se points bhejne ka function
    ...
    // Subscription buy karne ka 100% working function
